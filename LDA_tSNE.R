...
for (i in 1:length(w1)) {
  for (j in 1:length(perp)) {
    ...
    file = sprintf(
      "~/Genome/LDA_tSNE/lda_tsne_perp_%s_w1_%s_w2_%s_w3_%s.png",
      perp[j],
      w1[i],
      w2[i],
      w3[i]
...
#     width = 1000,
#     height = 1000,)
# plot(tsne$Y[comp,], t = 'n', main = title)
# # legend("bottomleft", legend = sort(unique(type)), col = c(1:25),pch = c(1:25)%/%6+15)
# legend(
#   "bottomleft",
#   legend = unique(class),
#   col = topo.colors(class_num),
#   pch = 19
# )
# # text(tsne$Y, labels = as.character(as.factor(label)),col = type)
# # points(tsne$Y,col = type_num, pch = (type_num%/%6)+15)
# points(tsne$Y[comp,], col = topo.colors(class_num), pch = 19)
# dev.off()
